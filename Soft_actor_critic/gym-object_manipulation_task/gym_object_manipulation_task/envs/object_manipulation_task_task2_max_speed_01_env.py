...
class object_manipulation_task_task2Env(gym.Env):
    ...
    def step(self, action):
        ...
        # Finish if the agent licks.
        done = bool(action_lick > 0.08)
    ...
    def reset(self):
        # Start position.
        x_pos_start = np.random.uniform(low=-self.reward_zone_x_size, high=self.reward_zone_x_size)
        y_pos_start = np.random.uniform(low=self.min_y_pos, high=self.min_y_pos + self.reward_zone_y_size)

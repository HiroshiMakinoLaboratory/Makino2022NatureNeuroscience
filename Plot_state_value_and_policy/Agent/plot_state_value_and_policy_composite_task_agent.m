...
for seed_num = 1:numel(behavior_composite_task_pretraining)
    clearvars -except learning_stage behavior_composite_task_pretraining value_function_agent concat_sum_speed_vec_each_pos_x_agent concat_sum_speed_vec_each_pos_y_agent seed_num
    
    if contains(learning_stage,'early')
        observation = behavior_composite_task_pretraining{seed_num}.early.observation;
        action = behavior_composite_task_pretraining{seed_num}.early.action;
        reward = behavior_composite_task_pretraining{seed_num}.early.reward;
    elseif contains(learning_stage,'late')
        observation = behavior_composite_task_pretraining{seed_num}.late.observation;
        action = behavior_composite_task_pretraining{seed_num}.late.action;
        reward = behavior_composite_task_pretraining{seed_num}.late.reward;
    end

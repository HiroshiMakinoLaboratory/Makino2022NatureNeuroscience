function get_action_value_cell_task2_animal
...
% Load 'tuning_task2' after running 'get_tuning_curve_task2_animal.m'.

load('animal_behavior_task2.mat')
load('animal_activity_task2.mat')

for animal_num = 1:numel(behavior_task2)
    clearvars -except behavior_task2 activity_task2 tuning_task2 animal_num action_value_cell_task2
    
    for session_num = 1:numel(behavior_task2{animal_num})
        clearvars -except behavior_task2 activity_task2 tuning_task2 animal_num session_num action_value_cell_task2
        ...
        clearvars -except behavior_task2 activity_task2 tuning_task2 animal_num session_num filtered_value_function action_value_function action_value_function_lick action_value_cell_task2
        
        GLM = activity_task2{animal_num}{session_num};
        lick_onset_cell_idx = tuning_task2{animal_num}{session_num}.lick_onset_cell_idx;
        tuning_lick_onset = tuning_task2{animal_num}{session_num}.tuning_lick_onset;
        ...
        for region_num = region_num_temp:region
            % Action value.
            for cell_num = 1:length(lick_onset_cell_idx{region_num})
                if isempty(action_value_function_lick) == 1
                    dot_product_lick_onset{region_num}(cell_num) = nan;
                    correlation_lick_onset{region_num}(cell_num) = nan;
                else
                    dot_product_lick_onset{region_num}(cell_num) = nansum(tuning_lick_onset{region_num}(lick_onset_cell_idx{region_num}(cell_num),:).*action_value_function_lick);
                    correlation_lick_onset{region_num}(cell_num) = corr(tuning_lick_onset{region_num}(lick_onset_cell_idx{region_num}(cell_num),:)',action_value_function_lick','row','complete');
                end
            end
        end
        
        action_value_cell_task2{animal_num}{session_num}.action_value_function = action_value_function;
        action_value_cell_task2{animal_num}{session_num}.action_value_dist = action_value_function_lick;
        action_value_cell_task2{animal_num}{session_num}.dot_product_lick = dot_product_lick_onset;
        action_value_cell_task2{animal_num}{session_num}.correlation_lick = correlation_lick_onset;
        action_value_cell_task2{animal_num}{session_num}.filtered_value_function = filtered_value_function;
    end
end
